...
try {
    ...
    if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
        ...
        if (($resultadoValidacion['errores'] ?? []) === []) {
            crearSolicitudUsuario($pdo, $datos);
            $mensaje = 'Tu solicitud de acceso se ha registrado correctamente. Queda pendiente de aprobación.';
        } else {
            $error = implode(' ', $resultadoValidacion['errores']);
        }
    }
} catch (Throwable $e) {
